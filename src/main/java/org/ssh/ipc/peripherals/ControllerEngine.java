package org.ssh.ipc.peripherals;
...
public abstract class ControllerEngine<
        A extends Agent,
        S extends Strategy<? extends A, ? extends Command, ? extends G>,
        ...
        this.controllerStatePublisher = Flux.create(fluxSink -> {
            while (true) {
                fluxSink.next(IntStream.range(0, controllerCount)
                        .boxed()
                        .filter(controllerIndex -> this.controllers.getControllerIndex(controllerIndex).isConnected())
    ...
    public Set<A> assignHandler(Integer handler, A agent) {
        ...
        return this.getPartsMap().put(handler, Sets.union(Collections.singleton(agent), this.getPartsMap().getOrDefault(handler, Collections.emptySet())));
    }
