 ...
 */
public class JsonSerializationContext implements SerializationContext<String, JsonSerializationContext, JsonSerializable> {
    /**
    ...
    private final GsonBuilder gsonBuilder = new Reflections("org.ssh")
            ...
            .filter(aClass -> Arrays.stream(aClass.getInterfaces()).anyMatch(JsonSerializer.class::isAssignableFrom))
            .reduce(
                    new GsonBuilder().setPrettyPrinting(),
                    (builder, serializerClass) -> {
                        try {
                            final JsonSerializer serializer = ((Class<JsonSerializer>) serializerClass).newInstance();
